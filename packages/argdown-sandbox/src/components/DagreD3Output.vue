  ...
  <div class="dagre-d3-output map-output output">
    <div class="content">
      <div ref="container" class="rendered">
        <svg ref="svgElement" width="100%" height="100%" style="display: block;"></svg>
      </div>
    </div>
  </div>
</template>
    ...
    function renderMap() {
      ...
      try {
        ...
        if (result && typeof result.catch === 'function') {
          result.catch((e) => console.log('Dagre render error:', e));
        }
      } catch (error) {
        console.log('Error rendering Dagre map:', error);
      }
    }
