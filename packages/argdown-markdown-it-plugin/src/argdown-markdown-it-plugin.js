...
const createArgdownPlugin = (config, customArgdownApplication) => {
    const argdown = customArgdownApplication || defaultArgdownApplication;
    const webComponentPlugin = argdown.getPlugin(WebComponentExportPlugin.name, "export-web-component");
    let currentConfig = !config || typeof config === "function" ? {} : config;
    const webComponentDefaults = webComponentPlugin.defaults;
    const ArgdownPlugin = (md) => {
        const generateWebComponent = (code, initialView, additionalSettings) => {
...
//# sourceMappingURL=argdown-markdown-it-plugin.js.map